...
void mixTable() {
  ...
  #elif defined( HEX6X ) // now quad x with pin 10,11,A0,A1
    motor[0] = 1000; // pin 3 .. not used -> D3
    motor[1] = PIDMIX(-1,-1,+1); //FRONT_R -> D5
    motor[2] = PIDMIX(+1,+1,+1); //REAR_L -> D6
    motor[3] = 1000; // pin 9.. not used -> D2
    motor[4] = PIDMIX(+1,-1,-1); //Left = FRONT_L -> D7
    motor[5] = PIDMIX(-1,+1,-1); //Right = REAR_R -> D8
  #elif defined( HEX6H )
    motor[0] = PIDMIX(-1,+1,-1); //REAR_R
    motor[1] = PIDMIX(-1,-1,+1); //FRONT_R
    motor[2] = PIDMIX(+ 1,+1,+1); //REAR_L
